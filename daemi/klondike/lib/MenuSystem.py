...
BORDER_LEFT  = Color(0xffffff50)
BORDER_RIGHT = Color(0x00000050)
BUTTON       = 1
SWITCH       = 0
...
class Menu(Rect,object):
    ...
    def __init__(self,label,itemslist,exc=()):
        ...
        self.lineheight = int(self.lineheight*SPACING)##########################################################################
    ...
    def draw(self):
        ...
        y += int(self.lineheight*(SPACING/4-0.25))###########################################################################
...
class MenuBar(MenuSystem,object):
    ...
    def __init__(self):
        self.lineheigth = int(FONT.get_height()*SPACING)#####################################################
        self.rect = Rect(0,0,DISPLAYRECT.width,self.lineheigth)

    def set(self,menulist=None):
        ...
        if menulist:
            ...
            for item in self.menulist:
                ...
                self.rects.append(Rect(x,0,w+self.lineheigth,self.lineheigth))#####################################################
    ...
    def update(self,ev):
        ...
        if ret and self.choice:
            self.choice = [(self.index,self.menulist[self.index].label)]+self.choice
            ...
            if not self or self.boxindex == None:
                if ev.type == MOUSEBUTTONUP:
                    if self.index > -1 and ev.button == 1:
                        if not ret:
                            ret = super(MenuBar,self).set(self.menulist[self.index],self.rects[self.index].bottomleft)
                    elif ret:
                        ret += [self.draw()]
    ...
    def draw(self):
        ...
        for item in self.menulist:
            x = DISPLAY.blit(FONT.render(item.label,1,FGCOLOR),(x,self.rect.y+self.lineheigth*(SPACING/4-0.25))).right+self.lineheigth#####################################################
        return self.rect
...
class MenuChoice(MenuSystem,object):
    ...
    def draw(self):
        ...
        DISPLAY.blit(FONT.render(self.menu.label,1,FGCOLOR),(x,self.rect.y+self.lineheigth*(SPACING/4-0.25)))#################################
        DISPLAY.set_clip(clipxy)
        return self.rect
...
class Button(Rect,object):
    ...
    def set(self,type=BUTTON,active=True,switch=False,switchlabel=None,font=None):
    ...
    def draw(self):
        def draw(bgcolor,fgcolor,topleftcolor,bottomrightcolor):
            gfxdraw.box(DISPLAY,self,bgcolor)
            gfxdraw.vline(DISPLAY,self.right-1,self.top,self.bottom-1,bottomrightcolor)
            gfxdraw.hline(DISPLAY,self.left,self.right-2,self.bottom-1,bottomrightcolor)
            gfxdraw.vline(DISPLAY,self.left,self.top,self.bottom-1,topleftcolor)
            gfxdraw.hline(DISPLAY,self.left+1,self.right-1,self.top,topleftcolor)
            clipxy = DISPLAY.get_clip()
            DISPLAY.set_clip(self.inflate(-2,-2))
            if self.type == SWITCH:
                label = self.font.render(self.label if not self.switch else self.switchlabel,1,fgcolor)
            else:
                label = self.font.render(self.label,1,fgcolor)
